...
- 감사 로그(`audit_logs`) — 실제로 기록되는 것은 다음뿐이다. 승인/반려 · 계정 생성/삭제/비활성화 ·
  첫 로그인 비밀번호 변경 · 시작 가이드 통과 · **조직 마스터** 업로드 · SME 명부 반영 ·
  Export 내려받기 · 스냅샷 · 메일 발송 · 운영 설정 변경.
  행위 이름과 남기는 위치는 `docs/OPERATIONS.md` §7-3에 있다.
  **제출·재제출과 직무정보(4시트) 업로드는 아직 `audit_logs`에 남지 않는다** — 제출은
  `review_history`에만 남고(제출 이력 자체는 추적된다), 직무정보 업로드는 어느 표에도 남지 않는다.
  §8 S5가 요구한 것과의 차이는 `docs/OPEN_ISSUES.md`의 「§8 S5 미이행 — 남은 작업」에 적어 두었다.
...
③ FTE 합계 100.00 ④ 호출자 = 배정 SME 본인을 다시 검증한 뒤에만 통과시킨다.
...
`supabase/seed.sql`을 SQL Editor에서 실행하면 화면 확인용 계정 둘이 생긴다.

- 관리자: `admin@jobreview.local` / `admin1234`
- SME: `sme@jobreview.local` / `sme1234`

**이 두 계정은 비밀번호가 이 문서에 공개되어 있으므로 운영 배포 전에 반드시 삭제한다.
운영 관리자 계정을 먼저 만들고 나서 지운다 — 순서를 바꾸면 관리자 없이 잠긴다.**
절차는 `docs/OPERATIONS.md` 3절에 있다. 운영 DB에서는 `seed.sql`을 다시 실행하지 않는다.
