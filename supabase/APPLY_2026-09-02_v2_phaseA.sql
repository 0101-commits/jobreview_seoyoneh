/* =====================================================================
   Job Review — 2026-09-02 v2 Phase A(복구) 운영 DB 적용 SQL (한 벌)

   ▣ 무엇을 적용하나
     20260902040000_v2_phaseA_recovery.sql 한 벌. 표·컬럼·함수 본문은 건드리지 않고
     권한만 줄인다.

       ⓐ profiles 컬럼 단위 UPDATE 권한 재정의 — email·organization·title 회수 (v2 S1)
       ⓑ request_rereview 실행 권한 회수 (v2 F8)

   ▣ 적용 후 확인 (SME 세션으로)
     PATCH /rest/v1/profiles?id=eq.<본인>  body {"organization":"x"}  → 42501 이어야 한다.
     PATCH … body {"name":"홍길동"}                                  → 200 이어야 한다.
     POST  /rest/v1/rpc/request_rereview                              → 42501 이어야 한다.
   ...
   ▣ 되돌리기
     ...
     — 되돌리면 S1·F8이 그대로 되살아난다. 관리자 화면의 계정 수정은 Edge Function이 하므로
       되돌릴 이유는 없다.
   ===================================================================== */
